...
{   
	rt_uint8_t hsareg,heareg,vsareg,veareg;
...
{ 	
    ...
    __HAL_RCC_AFIO_CLK_ENABLE();
 	...
 	GPIO_InitStructure.Pin = GPIO_PIN_0;				 //PB0 推挽输出 背光
 	GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP; 		 //推挽输出
 	GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_HIGH;
 	HAL_GPIO_Init(GPIOB, &GPIO_InitStructure);
	
     	//PORTD复用推挽输出  
	GPIO_InitStructure.Pin = GPIO_PIN_0|GPIO_PIN_1|GPIO_PIN_4|GPIO_PIN_5|GPIO_PIN_8|GPIO_PIN_9|GPIO_PIN_10|GPIO_PIN_14|GPIO_PIN_15;				 //	//PORTD复用推挽输出  
 	GPIO_InitStructure.Mode = GPIO_MODE_AF_PP; 		 //复用推挽输出   
 	GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_HIGH;
 	HAL_GPIO_Init(GPIOD, &GPIO_InitStructure); 
	  
	//PORTE复用推挽输出  
	GPIO_InitStructure.Pin = GPIO_PIN_7|GPIO_PIN_8|GPIO_PIN_9|GPIO_PIN_10|GPIO_PIN_11|GPIO_PIN_12|GPIO_PIN_13|GPIO_PIN_14|GPIO_PIN_15;				 //	//PORTD复用推挽输出  
 	GPIO_InitStructure.Mode = GPIO_MODE_AF_PP; 		 //复用推挽输出   
 	GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_HIGH;
 	HAL_GPIO_Init(GPIOE, &GPIO_InitStructure); 
	  
   	//	//PORTG12复用推挽输出 A0	
	GPIO_InitStructure.Pin = GPIO_PIN_0|GPIO_PIN_12;	 //	//PORTD复用推挽输出  
 	GPIO_InitStructure.Mode = GPIO_MODE_AF_PP; 		 //复用推挽输出   
 	GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_HIGH;
	...
	writeTiming.AddressSetupTime = 0x00;	 //地址建立时间（ADDSET）为1个HCLK  
    writeTiming.AddressHoldTime = 0x00;	 //地址保持时间（A		
    writeTiming.DataSetupTime = 0x03;		 ////数据保存时间为4个HCLK	
    ...
    FSMC_NORSRAMInitStructure.NSBank = FSMC_NORSRAM_BANK4;//  这里我们使用NE4 ，也就对应BTCR[6],[7]。
    FSMC_NORSRAMInitStructure.DataAddressMux = FSMC_DATA_ADDRESS_MUX_DISABLE; // 不复用数据地址
    FSMC_NORSRAMInitStructure.MemoryType =FSMC_MEMORY_TYPE_SRAM;// FSMC_MemoryType_SRAM;  //SRAM   
    FSMC_NORSRAMInitStructure.MemoryDataWidth = FSMC_NORSRAM_MEM_BUS_WIDTH_16;//存储器数据宽度为16bit   
    FSMC_NORSRAMInitStructure.BurstAccessMode =FSMC_BURST_ACCESS_MODE_DISABLE;// FSMC_BurstAccessMode_Disable; 
    ...
    FSMC_NORSRAMInitStructure.WriteOperation = FSMC_WRITE_OPERATION_ENABLE;	//  存储器写使能
    ...
    FSMC_NORSRAM_Init(FSMC_Bank1, &FSMC_NORSRAMInitStructure);//初始化FSMC配置, 使能BANK1 
	...
	}else if(lcddev.id==0X9331)//OK |/|/|			 
	{
		...
		rt_thread_mdelay(200); // Dis-charge capacitor power voltage
