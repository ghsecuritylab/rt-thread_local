...
static void enc28j60(void)
{
    ...
    NET_DEBUG("-- enc28j60 registers:\n");
    NET_DEBUG("HwRevID: 0x%02X\n", spi_read(spi_device, EREVID));

    NET_DEBUG("Cntrl: ECON1 ECON2 ESTAT  EIR  EIE\n");
    NET_DEBUG("       0x%02X  0x%02X  0x%02X  0x%02X  0x%02X\n",
    ...
    NET_DEBUG("MAC  : MACON1 MACON3 MACON4\n");
    NET_DEBUG("       0x%02X   0x%02X   0x%02X\n",
               spi_read(spi_device, MACON1),
               spi_read(spi_device, MACON3),
               spi_read(spi_device, MACON4));

    NET_DEBUG("Rx   : ERXST  ERXND  ERXWRPT ERXRDPT ERXFCON EPKTCNT MAMXFL\n");
    NET_DEBUG("       0x%04X 0x%04X 0x%04X  0x%04X  ",
    ...
    NET_DEBUG("0x%02X    0x%02X    0x%04X\n",
               spi_read(spi_device, ERXFCON),
               spi_read(spi_device, EPKTCNT),
               (spi_read(spi_device, MAMXFLH) << 8) | spi_read(spi_device, MAMXFLL));

    NET_DEBUG("Tx   : ETXST  ETXND  MACLCON1 MACLCON2 MAPHSUP\n");
    NET_DEBUG("       0x%04X 0x%04X 0x%02X     0x%02X     0x%02X\n",
    ...
    NET_DEBUG("PHY   : PHCON1 PHSTAT1\r\n");
    NET_DEBUG("        0x%04X 0x%04X\r\n",
