...
/*以下是图形GUI应用*/
void GUI_clearscreen(u32 b_color)
{
	u32 i,j;
	...
	{
		for(j=0;j<240;j++)
			LCD_WriteRAM(b_color);
	}
}

void GUI_Point(u8 x, u32 y, u32 color)
{   
	LCD_SetCursor(x,y);		//设置光标位置 
	LCD_WriteRAM_Prepare();	//开始写入GRAM
	LCD->LCD_RAM=color; 
...
以二进制形式显示出来。注意字符是8*16点阵，240/8=30 320/16=20
************************************************************/
void GUI_sprintf_chartobit(u8 x, u32 y,u8 dat, u32 color,u32 b_color)
{  
	u8 i ;
	for(i=0;i<8;i++)
	{
...
说明：无符号字符型数用指定的颜色和背景颜色在指定的坐标位置上
      以十六进制形式显示出来。
************************************************************/
void GUI_sprintf_chartohex(u8 x, u32 y,u8 dat, u32 color,u32 b_color)
{  
//	int i ;
...
说明：无符号字符型数用指定的颜色和背景颜色在指定的坐标位置上
      以十进制形式显示出来。
************************************************************/
void GUI_sprintf_chartodec(u8 x,u32 y,u8 dat,u32 color,u32 b_color)
{
...
说明：用指定位置上显示1位16进制数。
**********************************************/
void GUI_sprintf_nu(u8 x, u32 y,u8 nu, u32 color,u32 b_color)
{  
...
************************************************************/
void GUI_HLine(u8 x, u32 y, u8 length, u32 color) 
{  
	LCD_Set_Window(x,y,x+length-1,y);
	do
	{  
		LCD_WriteRAM(color);//????,?????
		length--;
...
************************************************************/
void GUI_RLine(u8 x, u32 y, u32 high, u32 color) 
{  
   LCD_Set_Window(x,y,x,y+high-1);
   do
   {  
		LCD_WriteRAM(color);// ????,?????
		high--;
...
***********************************************************************/
void GUI_line(u32 x1,u32 y1,u32 x2,u32 y2,u32 color)  
{  
    u32 t;  
    int xerr=0,yerr=0,delta_x,delta_y,distance;  
    int incx,incy;  
...
************************************************************/
void GUI_DisPicture(u8 x, u32 y, u8 length, u32 high ,const u8 *pic)
{
   u32 temp=0,tmp=0,num=0;
...
**********************************************/
void GUI_sprintf_char(u8 x, u32 y,u8 c, u32 color,u32 b_color)
{  
	u8  s_x ,s_y, temp ;
  	u32 j;
  	c -= 32;
...
说明：在指定位置上显示英文字符串。
**********************************************/
void GUI_sprintf_string(u8 x, u32 y,char *s, u32 color,u32 b_color)
{  
	for(;*s!='\0';s++)
	{
...
/*********************************************
显示预定义汉字字符串,汉字是16*16，所以1行最多显示240/16=15字，320/16=20行
...
说明：在指定位置上显示预定义的中文字符串。
      要显示的中文字符串事先定义在chinese.h中的china_char[]数组中，
		...
		中文字符串自动转换为数组数据。
**********************************************/
void GUI_sprintf_HZ1(u8 x, u32 y, u32 color,u32 b_color)
{ 
	u8  s_x=0 ,s_y=0, temp=0 ;  
	u8 n;
	u32 j,words;
...
void GUI_sprintf_HZ2(u8 x, u32 y, u32 color,u32 b_color)
{ 
	u8  s_x=0 ,s_y=0, temp=0 ;  
	u8 n;
	u32 j,words;
...
说明：在指定位置上画出实心矩形。
**********************************************/
void GUI_box(u8 sx,u32 sy,u8 ex,u32 ey,u32 color)
{ 
	u32 temp;   
...
说明：在指定位置上画出空心矩形。
**********************************************/
void GUI_rectangle( u8 sx,u32 sy,u8 ex,u32 ey,u32 color)
{
...
注意：在使用该函数时，不论最终显示的数有几位，它都将在LCD屏幕上
      占10个字符的空间（显示的多位十进制数不足10位时，
		本函数会在有效显示数字前面补空格）。
**********************************************/
void GUI_wrul(u8 x, u32 y, unsigned long num, u32 color,u32 b_color)
{
	unsigned long temp; 
	u8 i;
	for(i=0;i<10;i++)
...
**********************************************/
void GUI_wrlval(u8 x, u32 y,unsigned long num,u8 bits,u8 dp,u32 color,u32 b_color)
{
...
{
	return((u32)(R&0XF8)<<8|(u32)(G&0XFC)<<3|(u32)(B&0XF8)>>3);
}

//画坐标轴
void GUI_xyFrame(void)
{
    unsigned int temp;
    unsigned int num,n; 
		...
		for(n=0;n<240;n++)
		{
			for(num=0;num<320;num++)
	         LCD_WriteRAM(temp);
               ...
               GUI_HLine(20, 294, 210, 0x0000);//?????X????
               GUI_RLine(20, 150, 144, 0x0000);//?????Y????
               GUI_RLine(40, 292, 2, 0x0000);//?????X???????
               GUI_RLine(60, 292, 2, 0x0000);//?????X???????
               GUI_RLine(80, 2922, 2, 0x0000);//?????X???????
               GUI_RLine(100, 292, 2, 0x0000);//?????X???????
               GUI_RLine(120, 292, 2, 0x0000);//?????X???????
               GUI_RLine(140, 292, 2, 0x0000);//?????X???????
               GUI_RLine(160, 292, 2, 0x0000);//?????X???????
               GUI_RLine(180, 292, 2, 0x0000);//?????X???????
               GUI_RLine(200, 292, 2, 0x0000);//?????X???????
               GUI_RLine(220, 292, 2, 0x0000);//?????X???????
              
               GUI_HLine(20, 166, 2, 0x0000);//?????Y?????
               GUI_HLine(20, 182, 2, 0x0000);//?????Y?????
               GUI_HLine(20, 198, 2, 0x0000);//?????Y?????
               GUI_HLine(20, 214, 2, 0x0000);//?????Y?????
               GUI_HLine(20, 230, 2, 0x0000);//?????Y?????
               GUI_HLine(20, 246, 2, 0x0000);//?????Y?????
               GUI_HLine(20, 262, 2, 0x0000);//?????Y?????
               GUI_HLine(20, 278, 2, 0x0000);//?????Y?????
							 
               GUI_line(225,289,230,294,0x0000) ;
               GUI_line(225,299,230,294,0x0000) ;      
               GUI_line(15,155,20,150,0x0000) ;
               GUI_line(25,155,20,150,0x0000) ; 
							 
               GUI_sprintf_char(16,295,'0',0x0000,0xffff);
               GUI_sprintf_char(36,295,'1',0x0000,0xffff);
               GUI_sprintf_char(56,295,'2',0x0000,0xffff);
               GUI_sprintf_char(76,295,'3',0x0000,0xffff);
               GUI_sprintf_char(96,295,'4',0x0000,0xffff);
               GUI_sprintf_char(116,295,'5',0x0000,0xffff);
               GUI_sprintf_char(136,295,'6',0x0000,0xffff);
               GUI_sprintf_char(156,295,'7',0x0000,0xffff);
               GUI_sprintf_char(176,295,'8',0x0000,0xffff);
               GUI_sprintf_char(196,295,'9',0x0000,0xffff);
               GUI_sprintf_char(216,295,'1',0x0000,0xffff);
               GUI_sprintf_char(224,295,'0',0x0000,0xffff);
               GUI_sprintf_char(0,271,'1',0x0000,0xffff);
               GUI_sprintf_char(8,271,'0',0x0000,0xffff);
               GUI_sprintf_char(0,255,'2',0x0000,0xffff);
               GUI_sprintf_char(8,255,'0',0x0000,0xffff);
               GUI_sprintf_char(0,239,'3',0x0000,0xffff);
               GUI_sprintf_char(8,239,'0',0x0000,0xffff);
               GUI_sprintf_char(0,223,'4',0x0000,0xffff);
               GUI_sprintf_char(8,223,'0',0x0000,0xffff);
               GUI_sprintf_char(0,207,'5',0x0000,0xffff);
               GUI_sprintf_char(8,207,'0',0x0000,0xffff);
               GUI_sprintf_char(0,191,'6',0x0000,0xffff);
               GUI_sprintf_char(8,191,'0',0x0000,0xffff);
               GUI_sprintf_char(0,175,'7',0x0000,0xffff);
               GUI_sprintf_char(8,175,'0',0x0000,0xffff);
               GUI_sprintf_char(0,159,'8',0x0000,0xffff);
               GUI_sprintf_char(8,159,'0',0x0000,0xffff); 
}
